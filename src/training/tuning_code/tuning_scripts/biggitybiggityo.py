...
from transformers import AutoModel, AutoTokenizer, AutoModelForSequenceClassification, \
                        TrainingArguments, Trainer, DataCollatorWithPadding, BitsAndBytesConfig, \
                         AutoConfig, PreTrainedModel, AutoModelForCausalLM
...
def set_model(checkpoint, tokenizer, ModelType=AutoModel):
    ...
    model = ModelType.from_pretrained(checkpoint, torch_dtype=torch.bfloat16, num_labels=7, 
                                     trust_remote_code=True, device_map=PartialState().process_index,
                                     quantization_config=quant_config, attn_implementation="flash_attention_2")
