...
import pandas as pd
import pytest
from data.preprocessing_scripts.scripts.utils import get_file_extension, read_data, save_data
...
def test_get_file_extension(dummy_source_file_path: Path, expected_file_extension: str) -> None:
    """Test the get_file_extension function."""
    got: str = get_file_extension(dummy_source_file_path)
...
def test_read_data(dummy_source_file_path: Path, expected_read_data: pd.DataFrame) -> None:
    """Test if read data works correctly."""
    got: pd.DataFrame = read_data(dummy_source_file_path)

    assert got.equals(expected_read_data)


def test_save_data(dummy_source_file_path, dummy_target_file_path) -> None:
    """Test if save_data works correctly and saves a file."""
    data: pd.DataFrame = read_data(dummy_source_file_path)

    save_data(data, dummy_target_file_path)
